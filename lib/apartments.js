...
(function (root, factory) {
   ...
   * per section only the dominant PROPERTY_427 (project name) is counted.
   */
  function aggregateProducts(products, sectionToProject, options) {
    ...
    for (const [sec, rows] of bySection) {
      ...
      const names = new Map();
      rows.forEach((p) => { const n = String(pval(p[o.product.projectName]) || ''); names.set(n, (names.get(n) || 0) + 1); });
      let dominant = '', best = -1;
      names.forEach((c, n) => { if (c > best) { best = c; dominant = n; } });
      const floors = projects[projectId] || (projects[projectId] = {});
      for (const p of rows) {
        if (String(pval(p[o.product.projectName]) || '') !== dominant) { stats.skippedForeignProject++; continue; }
