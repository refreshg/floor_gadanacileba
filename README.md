...
- **სულ ბინა** — ცხრილის ბოლო ხაზში მენეჯერის ბინების ჯამი ხილულ პროექტებზე (ფილტრზე რეაგირებს): CRM კატალოგიდან,
  ყველა ბინა სტატუსის მიუხედავად, პარკინგის გარეშე. ქვემოთ პატარა ტექსტით სართულების და პროექტების რაოდენობა.
  ბინების მონაცემები ფონში იტვირთება (ჰოსტინგზე 30 წუთი cache-ში); სათაურში ჩანს «ბინები HH:MM». თუ ბინები ვერ ჩაიტვირთა,
  ხაზი სართულების ჯამს აჩვენებს. `*` = პროექტი კატალოგის სექციაზე მიბმული არ არის (ლისტი 111, sectionID).
...
1. Vercel → პროექტი → **Settings → Environment Variables**:
   ...
   - `BITRIX_CRM_WEBHOOK` = `crm` scope-ის webhook (ბინების ჯამისთვის; მხოლოდ კითხვა:`crm.product.list`)
...
- `config.js` — webhook URL, `crmWebhook` (ბინებისთვის), `iblockId`, `iblockType` (ლოკალური, git-ში არ იდება).
...
- ბინების ჯამი cache-ის გამო 30 წუთამდე ძველ მონაცემებს ეყრდნობა. კატალოგის სრული წაკითხვა ≈10 წამი.
- მენეჯერების ჯამების ჯამი ≠ ბინების საერთო რაოდენობა: არმიკუთვნებული სართულები არ ითვლება, ორ მენეჯერზე გადაფარული სართული ორივეს ჯამში შედის.
